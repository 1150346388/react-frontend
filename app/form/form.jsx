...
import Utils from '../collection/collection-utils';
...
export default class Form extends React.Component{
	...
	initForm(){
        ...
        let checkValue = [];
        let pname;
        for (let i = 0; i< this.refs.form && this.refs.form.length; i++) {
    ...
    initFormValidation(){
        ...
        // 初始化验证规则
        // 获取实体的 所有 验证规则
        let rulesAll = config[entity]['entity']['rules'] || {};
        // 获取实体的 所有 报错信息
        let msgAll = config[entity]['entity']['messages'] || {};
    ...
    formOperation(datas){
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                // 数据操作成功后 发布事件
                // PubSub.publish('collection.task.addtask',{});
                resolve();
            }, 5000);
        });
	...
	render(){
		...
		return(
			<form ref="form" key={Utils.getKey('form')} {...others}>
